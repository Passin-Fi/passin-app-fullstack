...
function OrderView({ dataOrder, isSuccessPayment, wallet }: { dataOrder: OrderDoc; isSuccessPayment: boolean; wallet: WalletInfo }) {
    ...
    const subscribeToPool = async () => {
        ...
        console.log({ smartWalletPubkey: smartWalletPubkey.toString(), wallet });
        const smartWallet = await checkSmartWallet(wallet.passkeyPubkey);
        console.log('Checked smart wallet:', smartWallet.smartWallet?.toString());
        if (!smartWallet.smartWallet) {
            toast.error('Smart wallet not found for your passkeys. Please try again later.');
            return;
        }
        try {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                console.log('syncWalletStatus done:');
                await sleep(150);
